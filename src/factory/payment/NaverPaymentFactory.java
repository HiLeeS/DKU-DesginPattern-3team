package factory.payment;

import strategy.payment.NaverPayment;
import strategy.payment.PaymentStrategy;

/**
 * 네이버페이 결제 팩토리 구현 클래스
 * Abstract Factory 패턴의 Concrete Factory
 * 
 * 네이버페이 결제 전략과 네이버페이 결제 검증을 제공하는 제품군을 생성합니다.
 */
public class NaverPaymentFactory implements PaymentPolicyFactory {
    @Override
    public PaymentStrategy createPaymentStrategy() {
        return new NaverPayment();
    }
    
    @Override
    public PaymentVerification createPaymentVerification() {
        return new NaverPaymentVerification();
    }
}

